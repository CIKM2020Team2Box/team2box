...
class Team2Box:
    ...
    def loadTeams(self,dataset):
        ...
        gfile=open(dataset+"/krnmdata1/teams.txt")
        gfile.readline()
        e=gfile.readline()
        self.Teams=[]
        while e:
    ...
    def loadG(self,dataset):
        self.dataset=dataset
        gfile=open(dataset+"/krnmdata1/teamsG.txt")
        #gfile.readline()
        e=gfile.readline()
        self.G={}
        ecount=1
    ...
    def CreateTeamG(self):        
        ...
        print(T,indx)
        indx=list(indx)
        tfile=open(self.dataset+"/krnmdata1/teams.txt","w")
        tfile.write("teamID expertID expertID ...\n")
        tqfile=open(self.dataset+"/krnmdata1/teamsquestions.txt","w")
        tqfile.write("teamID questionID questionID ...\n")
        qfile=open(self.dataset+"/krnmdata1/teamsG.txt","w")
        lenT=len(T)
        ...
        for i in range(lenT):
            ...
            if i+1!=lenT:
                ...
                while(j<lenT):  
                    ...
                    j+=1             
                #qfile.write("\n")      
        ebfile=open(self.dataset+"/krnmdata1/ExpertBestQuetionAnswer.txt","w")
    ...
    def run(self,walklen):
        ...
        loss_total=0
        train_len=len(pairs)
        logfile=open(self.dataset+"/krnmdata1/Team2boxlog.txt","w")
    ...
    def save_team_embedding(self,i):
        ...
        np.savetxt(self.dataset+"/krnmdata1/teamsembeding_"+str(i)+".txt",w1, fmt='%f')
        np.savetxt(self.dataset+"/krnmdata1/teamsOffsets_"+str(i)+".txt",offsets, fmt='%f')
        #qfile.close()
        
      
dataset=["android.stackexchange.com","history/history.stackexchange.com","dba/dba.stackexchange.com/","physics","english","electronics","softwareengineering","apple/apple.stackexchange.com"]
ob=Team2Box(32,dataset[7])
#ob.run(5)  

#ob=Team2Box(2,dataset[2])
#ob.CreateTeamG()
#ob.run_adam(5)
#ob.save_team_embedding()
#ob2=ExpertsEmbeding(2,dataset[2])
#ob2.run(13)
#w=ob2.walks1(13)
#print(w)
#print(ob2.get_train_pair(w,3,ob2.enum))